...
export async function performHandshake(socket, identity) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const onMessage = (message) => {
            ...
            try {
                const peerHandshake = message;
                validateHandshake(peerHandshake);
                const peerFingerprint = fingerprintFromPublicKey(peerHandshake.edPublicKey);
                session.setVerified(peerHandshake.edPublicKey, peerFingerprint, peerHandshake.displayName);
...
function validateHandshake(handshake) {
    ...
    if (diff > TIMESTAMP_TOLERANCE_MS) {
        throw new Error(`Handshake timestamp out of range: ${diff}ms (max ${TIMESTAMP_TOLERANCE_MS}ms)`);
    }
