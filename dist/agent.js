...
import { generateIdentity, fingerprintFromPublicKey, encrypt, decrypt, deriveKey, } from '@networkselfmd/core';
import { MessageType } from '@networkselfmd/core';
import { createId } from '@paralleldrive/cuid2';
import { AgentDatabase, IdentityRepository, PeerRepository, GroupRepository, MessageRepository, SenderKeyRepository, DiscoveredGroupRepository, } from './storage/index.js';
...
export class Agent extends EventEmitter {
    ...
    async start() {
        ...
        // Join TTYA topic
        const ttyaTopic = deriveKey(this.identity.edPublicKey, 'networkselfmd-ttya-v1', '', 32);
        await this.swarm.join(Buffer.from(ttyaTopic));
        // Join global network discovery topic
        const networkTopic = deriveKey(new TextEncoder().encode('networkselfmd'), 'networkselfmd-discovery-v1', '', 32);
    ...
    async sendDirectMessage(peerPublicKey, content) {
        const pk = hexToBytes(peerPublicKey);
    ...
    makeGroupPublic(groupId, selfMd) {
        const gid = hexToBytes(groupId);
    ...
    async loadOrGenerateIdentity() {
        ...
        if (stored) {
            let edPrivateKey = new Uint8Array(stored.ed_private_key);
            const edPublicKey = new Uint8Array(stored.ed_public_key);
            // If passphrase-protected, decrypt
            if (this.options.passphrase) {
                const keyData = this.identityRepo.loadEncryptedKeys();
                if (keyData) {
                    const wrappingKey = await deriveWrappingKey(this.options.passphrase, new Uint8Array(keyData.salt));
                    edPrivateKey = decrypt(wrappingKey, new Uint8Array(keyData.nonce), new Uint8Array(keyData.ciphertext));
                }
            }
    ...
    setupSwarmEvents() {
        this.swarm.on('peer:connected', (result) => {
        ...
        this.swarm.on('peer:verified', (result) => {
        ...
        this.swarm.on('peer:disconnected', (info) => {
    ...
    setupRouterHandlers() {
        ...
        router.on(MessageType.SenderKeyDistribution, (_session, message) => {
            this.groupManager.handleSenderKeyDistribution(message);
        });
        router.on(MessageType.GroupMessage, (session, message) => {
            this.groupManager
                .handleGroupMessage(session, message)
                .catch((err) => {
                this.emit('error', err);
            });
        });
        router.on(MessageType.GroupManagement, (session, message) => {
            this.groupManager.handleGroupManagement(session, message);
        });
        router.on(MessageType.DirectMessage, (session, message) => {
            this.handleDirectMessage(session, message);
        });
        router.on(MessageType.NetworkAnnounce, (session, message) => {
            ...
            for (const g of announce.groups) {
                this.discoveredGroupRepo.upsert(g.groupId, g.name, g.selfMd, g.memberCount, session.peerPublicKey);
            }
    ...
    handleDirectMessage(session, message) {
        if (!session.peerPublicKey)
            return;
        // Decrypt the message (simplified - in production use DoubleRatchet)
        let plaintext;
        try {
            plaintext = decrypt(session.peerPublicKey.subarray(0, 32), message.nonce, message.ciphertext);
        }
        catch {
